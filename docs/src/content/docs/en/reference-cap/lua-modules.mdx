...
description: How `lua_module_*` and `lua_driver_*` components register with `cap_lua`, how to author your own, and an overview of `display` ownership.
---
...
| `display` | `lua_module_display` | LCD drawing and touch input |
...
| `lcd` | `lua_module_lcd` | Optional LCD panel initialization, reset, and metadata; not used by `display` |
...
The registrar binds the Lua module name to its `luaopen_*` entry and may perform module-specific setup. `lua_module_display_register()` also registers script-exit cleanup.

**Register modules before the Lua capability group initializes.** Group initialization locks module registration; later calls fail.

### Example wiring from the app

```c
// Minimal app registration example
#include "lua_module_display.h"
#include "cap_lua.h"

esp_err_t register_display_lua_capability(void)
{
    esp_err_t err = lua_module_display_register();
...
`display.open(...)` acquires exclusive ownership of the built-in screen and returns a screen object. `screen:close()`, `<close>`, or script-exit cleanup releases ownership. Draw between `screen:begin()` and `screen:present()`; opening an already-open screen fails.

For long-running scenes, `lua_run_script_async` can use `exclusive:"display"` and `replace:true` to stop the previous display job before starting another.
