...
`cap_lua` accepts resolved absolute script paths. Callers derive paths from the active storage roots: writable scripts live under the DATA root's `scripts/`, firmware resources live under SYSTEM, and App entries come from the App registry.

- `path` must target a `.lua` file.
- `path` must be absolute and must not contain `..`; writable paths must not assume that the DATA root is always `/fatfs`.
- Skill documents use `{CUR_SKILL_DIR}/scripts/...` for their own scripts; Apps use the package-local `entry` validated by the registry.
...
// ...
cap_lua_register_group();  // start the capability group, then lock
```
