...
description: lua_module_* 与 lua_driver_* 组件的注册机制、实现方式，以及 display 模块的所有权说明。
---
...
| `display` | `lua_module_display` | LCD 屏幕绘图与触摸数据读取 |
...
| `lcd` | `lua_module_lcd` | 可选的 LCD 面板初始化、复位与信息读取；不供 `display` 使用 |
...
注册函数将 Lua 模块名与对应的 `luaopen_*` 入口关联，也可以完成模块自身所需的初始化。`lua_module_display_register()` 还注册脚本退出时的清理函数。

**注意：必须在 Lua capability group 初始化前注册模块。** group 初始化时会锁定模块注册，之后注册会被拒绝。
...
`lua_module_display` 通过 `display.open(...)` 独占内置屏，返回 screen 对象；`screen:close()`、`<close>` 或脚本退出清理时释放所有权。绘制在 `screen:begin()` 与 `screen:present()` 之间完成，重复打开报错。

长时间绘制的场景可通过 `lua_run_script_async` 的 `exclusive:"display"` 和 `replace:true` 停止上一个 display job，再启动新任务。
