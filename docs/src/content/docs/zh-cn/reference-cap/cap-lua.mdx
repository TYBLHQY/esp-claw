...
`cap_lua` 接受已解析的绝对脚本路径。调用方必须从当前存储根生成路径：可写脚本位于 DATA 根的 `scripts/`，固件资源位于 SYSTEM 根，App 入口由 App registry 提供。

- `path` 必须是 `.lua` 文件。
- `path` 必须是绝对路径且不能包含 `..`；可写路径不得假定 DATA 根固定为 `/fatfs`。
- Skill 文档用 `{CUR_SKILL_DIR}/scripts/...` 引用自身脚本；App 使用 registry 已校验的包内 `entry`。
...
// ...
cap_lua_register_group();  // 启动 capability group，之后锁定
```
